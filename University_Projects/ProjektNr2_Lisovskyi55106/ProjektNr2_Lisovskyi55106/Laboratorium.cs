...
namespace ProjektNr2_Lisovskyi55106
{
    public partial class Laboratorium : Form
    {
        ...
        public Laboratorium()
        {
            ...
            this.Location = new Point(Screen.PrimaryScreen.Bounds.X + SLMargines, Screen.PrimaryScreen.Bounds.Y + SLMargines);
        }
        ...
        {
            DialogResult SLWynikDialogu = MessageBox.Show("Czy rzeczywiście chcesz zakończyć działanie całego programu?", this.Text, MessageBoxButtons.YesNoCancel, MessageBoxIcon.Question, MessageBoxDefaultButton.Button3);

            if (SLWynikDialogu != DialogResult.Yes)
                e.Cancel = true;
            else
                e.Cancel = false;
